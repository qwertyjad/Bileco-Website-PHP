...
// Validate CSR program ID
if (!isset($_GET['id']) || empty($_GET['id'])) {
    echo "<script>alert('Invalid CSR program ID.'); window.location.href='csr-programs.php';</script>";
    exit();
}

$csr_program_id = $_GET['id'];
$csr_program = $function->getCSRProgramById($csr_program_id);

$csr_program_id = isset($csr_program_id) ? intval($csr_program_id) : 0;
$comment_count = $function->getCommentCount($csr_program_id);

// Fetch previous and next posts
$prevPost = $function->getPreviousPost($csr_program_id);
$nextPost = $function->getNextPost($csr_program_id);

// Fetch related posts based on category
$relatedPosts = $function->getRelatedPosts($csr_program_id);
...
// If CSR program not found, redirect
if (!$csr_program) {
    echo "<script>alert('CSR Program not found.'); window.location.href='csr-programs.php';</script>";
    exit();
}
...
<title><?php echo htmlspecialchars($csr_program['title']); ?></title>
    ...
    <div class="container mx-auto py-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="col-span-3 bg-white p-6 rounded-md">
            ...
            <nav class="text-sm mb-4">
                <a href="csr-programs.php" class="text-blue-600 hover:underline">CSR Programs</a> >
                <span class="text-gray-500"><?php echo htmlspecialchars($csr_program['title']); ?></span>
            </nav>

            <!-- Featured Image -->
            <?php if (!empty($csr_program['image'])): ?>
                <img src="data:image/jpeg;base64,<?php echo base64_encode($csr_program['image']); ?>"
                    class="w-full h-auto object-cover rounded-md shadow-md mb-6">
            <?php endif; ?>

            <!-- CSR Program Title -->
            <h1 class="text-3xl font-bold text-gray-800 mb-2">
                <?php echo htmlspecialchars($csr_program['title']); ?>
            </h1>
            <hr class="m-5">
            ...
            <div class="text-gray-500 text-sm p-5 flex items-center space-x-4">
                ...
                <span><i class="bx bx-calendar"></i> <?php echo date("F d, Y", strtotime($csr_program['date'])); ?></span>
            ...
            <div class="text-gray-700 text-base leading-relaxed mb-6 text-justify">
                <?php echo nl2br(htmlspecialchars($csr_program['content'])); ?>
            </div>

            <!-- Previous & Next Post Navigation -->
            <div class="flex justify-between border-t pt-4">
                <?php if ($prevPost): ?>
                    <a href="csr-programs-details.php?id=<?php echo $prevPost['id']; ?>" class="text-blue-600 hover:underline">
                ...
                <?php if ($nextPost): ?>
                    <a href="csr-programs-details.php?id=<?php echo $nextPost['id']; ?>" class="text-blue-600 hover:underline">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <?php foreach ($relatedPosts as $post): ?>
                    <a href="csr-programs-details.php?id=<?php echo $post['id']; ?>" class="block bg-white p-4 rounded-md shadow-md hover:shadow-lg transition-shadow">
                        <div>
            ...
            <?php if ($user_status === 'online'): ?>
                <div class="mt-10 p-5 bg-gray-100 rounded-md">
                    ...
                    <form action="submit_comment.php" method="POST">
                        <input type="hidden" name="csr_program_id" value="<?php echo $csr_program_id; ?>">
