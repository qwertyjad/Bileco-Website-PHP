...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    echo "<script>alert('Invalid announcement ID.'); window.location.href='announcements.php';</script>";
    exit();
}

$announcement_id = $_GET['id'];
$announcement = $function->getAnnouncementById($announcement_id);

$announcement_id = isset($announcement_id) ? intval($announcement_id) : 0;
$comment_count = $function->getCommentCount($announcement_id);

// Fetch previous and next posts
$prevPost = $function->getPreviousPost($announcement_id);
$nextPost = $function->getNextPost($announcement_id);

// Fetch related posts based on category
$relatedPosts = $function->getRelatedPosts($announcement_id);
...
if (!$announcement) {
    echo "<script>alert('Announcement not found.'); window.location.href='announcements.php';</script>";
    exit();
}
    ...
    <div class="container mx-auto py-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="col-span-3 bg-white p-6 rounded-md">
            ...
            <nav class="text-sm mb-4">
                <a href="announcements.php" class="text-blue-600 hover:underline">Announcements</a> >
                <span class="text-gray-500"><?php echo htmlspecialchars($announcement['title']); ?></span>
            </nav>

            <!-- Featured Image -->
            <?php if (!empty($announcement['image'])): ?>
                <img src="data:image/jpeg;base64,<?php echo base64_encode($announcement['image']); ?>"
            ...
            <div class="text-gray-500 text-sm p-5 flex items-center space-x-4">
                ...
                <span><i class="bx bx-calendar"></i> <?php echo date("F d, Y", strtotime($announcement['date'])); ?></span>
            ...
            <div class="flex justify-between border-t pt-4">
                <?php if ($prevPost): ?>
                    <a href="announcement-details.php?id=<?php echo $prevPost['id']; ?>" class="text-blue-600 hover:underline">
                ...
                <?php if ($nextPost): ?>
                    <a href="announcement-details.php?id=<?php echo $nextPost['id']; ?>" class="text-blue-600 hover:underline">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <?php foreach ($relatedPosts as $post): ?>
                    <a href="announcement-details.php?id=<?php echo $post['id']; ?>" class="block bg-white p-4 rounded-md shadow-md hover:shadow-lg transition-shadow">
                        <div>
