...
$item = $id ? $functions->getUncategorizedById($id) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        $success = $functions->updateUncategorized($id, $title, $content, $date, $image);
        $message = $success ? "Uncategorized entry updated successfully!" : "Failed to update Uncategorized entry.";
    } else {
        // Add new item
        $success = $functions->addUncategorized($title, $content, $date, $image);
        $message = $success ? "Uncategorized entry added successfully!" : "Failed to add Uncategorized entry.";
    }
...
<head>
    ...
    <title><?php echo $id ? 'Edit' : 'Add'; ?> Uncategorized</title>
                ...
                <div class="bg-white">
                    <div class="max-w-6xl mx-auto">
                        <h2 class="text-xl md:text-2xl font-semibold text-gray-800 mb-6 flex items-center">
                            <i class='bx bx-category-alt text-blue-600 text-xl md:text-2xl mr-2'></i>
                            <?php echo $id ? 'Edit' : 'Add'; ?> Uncategorized
                        </h2>
