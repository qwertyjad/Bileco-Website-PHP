...
$item = $id ? $functions->getPowerRateById($id) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        $success = $functions->updatePowerRate($id, $title, $content, $date, $image);
        $message = $success ? "Power Rate updated successfully!" : "Failed to update Power Rate.";
    } else {
        // Add new item
        $success = $functions->addPowerRate($title, $content, $date, $image);
        $message = $success ? "Power Rate added successfully!" : "Failed to add Power Rate.";
    }
...
<head>
    ...
    <title><?php echo $id ? 'Edit' : 'Add'; ?> Power Rate</title>
                ...
                <div class="bg-white">
                    <div class="max-w-6xl mx-auto">
                        <h2 class="text-xl md:text-2xl font-semibold text-gray-800 mb-6 flex items-center">
                            <i class='bx bx-dollar text-blue-600 text-xl md:text-2xl mr-2'></i>
                            <?php echo $id ? 'Edit' : 'Add'; ?> Power Rate
                        </h2>
...
<form method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="flex space-x-4">
        <button type="submit" 
            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-sm font-medium flex items-center">
            <i class='bx bx-save text-lg mr-2'></i> Save
        </button>
        <a href="power-rate.php" 
